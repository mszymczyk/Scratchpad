...
namespace misz.StatsViewer
{
    ...
    public class SessionControl : CanvasControl, IPerformanceTarget
    {
        /// <summary>
        ...
        public SessionControl(ISessionDocument timelineDocument)
		...
		//public StatsViewerD2dControl(ISessionDocument timelineDocument, DefaultStatsD2dRenderer timelineRenderer)
		//	: this(timelineDocument, timelineRenderer)
		//{
		//}
		...
		//public StatsViewerD2dControl(
		//	ISessionDocument timelineDocument,
		//	DefaultStatsD2dRenderer timelineRenderer
		//	)
		//	: this(timelineDocument, timelineRenderer)
		//{
		//}
			...
			)
        {
            ...
            {
                timelineRenderer = new DefaultSessionRenderer();
                timelineRenderer.Init(D2dGraphics);
            }
            m_statsRenderer = timelineRenderer;

            timelineRenderer.Invalidated += timelineRenderer_Invalidated;

            SessionDocument = timelineDocument;
        ...
        public override Matrix Transform
        {
            get
            {
                ...
                t.Translate(m_statsRenderer.HeaderWidth, 0, MatrixOrder.Append);
                return t;
            }
        }
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                m_statsRenderer.Invalidated -= timelineRenderer_Invalidated;
            }
        ...
        public override Rectangle VisibleClientRectangle
        {
            get
            {
                ...
                rect.X += m_statsRenderer.HeaderWidth;
                rect.Width -= m_statsRenderer.HeaderWidth;
                return rect;
            }
        }
        ...
        public ISessionDocument SessionDocument
        {
            get { return m_sessionDocument; }
            set
            {
                if (m_sessionDocument != null)
                {
					//m_selection.SelectionChanged -= selection_SelectionChanged;

                    m_sessionDocument.ItemInserted -= timelineDocument_ItemInserted;
                    m_sessionDocument.ItemRemoved -= timelineDocument_ItemRemoved;
                    m_sessionDocument.ItemChanged -= timelineDocument_ItemChanged;
                    m_sessionDocument.Reloaded -= timelineDocument_Reloaded;
                }
                ...
                m_sessionDocument = value;

                if (m_sessionDocument != null)
                {
                    m_session = m_sessionDocument.Session;
                    m_sessionDocument.ItemInserted += timelineDocument_ItemInserted;
                    m_sessionDocument.ItemRemoved += timelineDocument_ItemRemoved;
                    m_sessionDocument.ItemChanged += timelineDocument_ItemChanged;
                    m_sessionDocument.Reloaded += timelineDocument_Reloaded;

					//m_selection = m_sessionDocument.Cast<ISelectionContext>();
					//m_selection.SelectionChanged += selection_SelectionChanged;

					m_transactionContext = m_sessionDocument.As<ITransactionContext>();
                }
                else
                {
                    m_session = null;
        ...
        {
            get { return SessionDocument != null ? SessionDocument.Session : null; }
        }
		...
		//public IEnumerable<SessionPath> EditableSelection
		//{
		//	get
		//	{
		//		if (m_selection == null)
		//			yield break;
		//		foreach (SessionPath selected in m_selection.GetSelection<SessionPath>())
		...
		//public SessionPath TargetGroup
		//{
        ...
        public SessionLayout GetLayout()
        {
            if (m_layout != null)
                return m_layout;

            SessionLayout result = m_statsRenderer.GetLayout(m_session, Transform, ClientRectangle);
		...
		//	IList<HitRecord> hits = m_statsRenderer.Pick(m_session, pickRect, transform, clientRectangle);
		...
		//public bool Select<T>(SessionPath item)
		//	where T : class, ITimelineObject
		//{
		//	Keys modifiers = ModifierKeys;

		//	List<SessionPath> filtered;
		//	if (KeysUtil.ClearsSelection(modifiers))
		//	{
		//		filtered = new List<SessionPath>(1);
		//		filtered.Add(item);
		//	}
		//	else
		//	{
		//		filtered = new List<SessionPath>();
		//		foreach (SessionPath path in m_selection.GetSelection<SessionPath>())
		//		{
		//			if (path.Last is T)
		//				filtered.Add(path);
		//		}
		...
		///// instead of calling ISession.CreateGroup().</summary>
		...
		//	return SessionDocument.Timeline.CreateGroup();
		//}
		...
		//public virtual bool IsEditable(SessionPath path)
		//{
		//	if (path == null)
		//		return false;
		//	ISession owningTimeline = GetOwningTimeline(path.Last);
		//	return
		//		owningTimeline == null ||
		...
		//public virtual ISession GetOwningTimeline(ITimelineObject obj)
		//{
		...
		///// in the associated ISessionDocument, including its resolved sub-documents</summary>
		//public IEnumerable<SessionPath> AllEvents
		//{
		//	get
		//	{
		//		foreach (SessionPath path in GetObjects<IEvent>(m_session))
		//			yield return path;
		//	}
		//}

		///// <summary>
		///// Gets an enumerator that goes through all the markers in the associated ISessionDocument, including
		///// its resolved sub-documents</summary>
		//public IEnumerable<SessionPath> AllMarkers
		//{
		//	get
		//	{
		//		foreach (SessionPath path in GetObjects<IMarker>(m_session))
		//			yield return path;
		//	}
		//}

		///// <summary>
		///// Gets an enumerator that goes through all of the tracks in the associated ISessionDocument, including
		///// its resolved sub-documents</summary>
		//public IEnumerable<SessionPath> AllTracks
		//{
		//	get
		//	{
		//		foreach (SessionPath path in GetObjects<ITrack>(m_session))
		//			yield return path;
		//	}
		//}

		///// <summary>
		///// Gets an enumerator that goes through all of the groups in the associated ISessionDocument, including
		///// its resolved sub-documents</summary>
		//public IEnumerable<SessionPath> AllGroups
		//{
		//	get
		//	{
		//		foreach (SessionPath path in GetObjects<IGroup>(m_session))
		//			yield return path;
		//	}
		//}
		...
		//public static IEnumerable<T> GetObjectsInOneDocument<T>(ISession timeline)
		...
		//public static IEnumerable<SessionPath> GetObjects<T>(ISession timeline)
		//	where T : ITimelineObject
		//{
		//	foreach (ITimelineObject item in GetObjectsInOneDocument<T>(timeline))
		//		yield return new SessionPath(item);

		//	foreach (SessionPath path in GetHierarchy(timeline))
		//	{
		...
		//				yield return new SessionPath(path) + new SessionPath(item);
		//		}
		//	}
		//}
		...
		//public static IEnumerable<SessionPath> GetHierarchy(ISession timeline)
		//{
		//	IHierarchicalTimeline root = timeline as IHierarchicalTimeline;
		//	if (root == null)
		//		return EmptyEnumerable<SessionPath>.Instance;
		...
		//private static IEnumerable<SessionPath> GetHierarchy(
		...
		//		yield return new SessionPath(lineage);

		//		IHierarchicalTimeline child = reference.Target;
		//		if (child != null &&
		//			!all.Contains(child))
		//		{
		//			foreach (SessionPath childPath in GetHierarchy(child, lineage, all))
        ...
        public static Matrix CalculateLocalToWorld(SessionPath path)
        {
		...
		//	IEnumerable<SessionPath> objects,
		//	out float worldMin,
		//	out float worldMax)
		//{
		...
		//	foreach (SessionPath path in objects)
		//	{
		...
		//			m_activeGroup = hitGroup != null ? new SessionPath(hitGroup) : null;
		//			m_activeTrack = hitTrack != null ? new SessionPath(hitTrack) : null;
		//		}
		//	}
		...
		//	{
		//		if (m_resizingHeader)
		//			m_statsRenderer.HeaderWidth = Math.Max(e.X, MinHeaderWidth);
		...
		//		if (m_selection != null)
		//		{
		//			List<SessionPath> newSelection;
		//			Keys modifiers = Control.ModifierKeys;

		//			if (KeysUtil.ClearsSelection(modifiers))
		//				newSelection = new List<SessionPath>();
		//			else
		//				newSelection = new List<SessionPath>(m_selection.GetSelection<SessionPath>());

		//			IList<HitRecord> hits =
		//				m_statsRenderer.Pick(m_session, SelectionRect, Transform, ClientRectangle);
        ...
        {
			SessionLayout layout = GetLayout();

            RectangleF result = new RectangleF();

			IEnumerator<KeyValuePair<SessionPath, RectangleF>> pairs = layout.GetEnumerator();
			if (pairs.MoveNext())
				result = pairs.Current.Value;

			while (pairs.MoveNext())
			{
				KeyValuePair<SessionPath, RectangleF> pair = pairs.Current;
				...
				if (!selectionOnly)
				{
					...
					//}
					//else
					if (pair.Key.Last is IGroup || pair.Key.Last is ITrack )
					{
		...
		//{
		//	int tolerance = m_statsRenderer.PickTolerance;
		...
		//private IEnumerable<SessionPath> HitRecordsToEvents(IEnumerable<HitRecord> hits)
		//{
        ...
        private readonly SessionRenderer m_statsRenderer;

        private ISessionDocument m_sessionDocument;
		...
		//private SessionPath m_activeTrack;
		//private SessionPath m_activeGroup;
        ...
        private SessionLayout m_layout;
