...
{
    /// <summary>
    /// Abstract base class for stats renderer. The class is designed to be instantiated
    /// once per StatsViewerControl.</summary>
    public abstract class SessionRenderer : IDisposable
    {
        /// <summary>
        /// Constructor</summary>
        protected SessionRenderer()
        ...
        protected SessionRenderer(Font font)
        {
            Font = font;
        ...
        /// <remarks>The supplied object is considered to be owned by this StatsD2dRenderer
        ...
        /// <returns>Bounding rectangles for all timeline objects, organized in a dictionary of SessionPath/RectangleF pairs</returns>
            ...
            Rectangle clientRectangle)
        {
            ...
            SessionLayout layout = Layout(timeline, c);
            return layout;
        }
        ...
        public void Print(
			...
			//SessionPath activeGroup,
			//SessionPath activeTrack,
            Matrix transform,
            ...
            Rectangle marginBounds)
        {
            ...
            try
            {
				...
				Draw( timeline, transform, clientRectangle );
			}
            finally
            {
                m_printing = false;
        ...
        /// <param name="clientRectangle">Bounds of displayed area of timeline, in screen space</param>
        /// <returns>Bounding rectangles for all timeline objects, organized in a dictionary of SessionPath/RectangleF pairs</returns>
        public virtual SessionLayout Draw(
            ISession timeline,
			//ISelectionContext selection,
			//SessionPath activeGroup,
			//SessionPath activeTrack,
            ...
            RectangleF clientRectangle)
        {
            ...
            try
            {
				...
				//foreach (SessionPath path in D2dTimelineControl.GetHierarchy(timeline))
		...
		//			SessionPath groupPath = path + group;
		...
		//				{
		//					SessionPath trackPath = path + track;
		...
		//private void DrawMarkers(SessionPath path, ISession timeline, ISelectionContext selection,
		//	Context c, SessionLayout layout, RectangleF clipBounds)
		//{
		//	RectangleF bounds;
		//	foreach (IMarker marker in timeline.Markers)
		//	{
		//		SessionPath markerPath = path + marker;
        ...
        private void DrawGroupsAndTracks(SessionPath path, ISession timeline, bool expandedTimeline,
			//ISelectionContext selection,
			Context c, SessionLayout layout, RectangleF clipBounds)
        {
                ...
                if (bounds.IntersectsWith(clipBounds))
                {
                    ...
                    foreach (ITrack track in tracks)
                    {
                        ...
                        if (bounds.IntersectsWith(clipBounds))
                        {
							...
							//{
							//	SessionPath intervalPath = path + interval;
		...
		//	Context c = new Context(this, transform, clientRectangle, m_graphics);
		//	SessionLayout layout = GetCachedLayout(timeline, c);
        ...
        protected SessionLayout Layout(ISession timeline, Context c)
        {
            SessionLayout result = new SessionLayout();
        ...
        protected SessionLayout GetCachedLayout(ISession timeline, Context c)
        {
		...
		//	SessionPath path, ref float documentTop, Context c,
		//	SessionLayout result)
		//{
		//	SizeF pixelSize = c.PixelSize;
        ...
        private void LayoutSubTimeline(
            SessionPath path,
            ISession timeline, ref float documentTop, bool expandedTimeline, Context c,
            SessionLayout result)
        {
            //if (c.TestRecursion(timeline))
            //    return;
        ...
        public class Context
        {
            ...
            //public bool TestRecursion(ISession timeline)
            //{
            ...
            private readonly HashSet<ISession> m_timelines = new HashSet<ISession>();
        }
        ...
        // For caching the layout dictionary for the last ISession that was drawn.
        //  Used to improve picking performance.
        private Pair<ISession, SessionLayout> m_cachedLayout;
    }
}
