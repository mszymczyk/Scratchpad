...
{
	public interface DynamicLongEnumEditorLister
	{
		string[] GetNames( PropertyEditorControlContext context );
	}
    ...
    /// A DynamicLongEnumEditor is created for each PropertyDescriptor, for each
    ...
    public class DynamicLongEnumEditor : IPropertyEditor, IAnnotatedParams
    {
        /// <summary>
        /// Default constructor. Use DefineEnum() to populate the list of items to choose from.</summary>
        public DynamicLongEnumEditor( DynamicLongEnumEditorLister lister )
        {
			m_lister = lister;
        }

		///// <summary>
		///// Construct DynamicLongEnumEditor using an enum type and option associated images</summary>
		///// <param name="enumType">The enum type</param>
		///// <param name="images">optional icons</param>
		//public DynamicLongEnumEditor(Type enumType, Image[] images = null)
		//{
		//	if (!enumType.IsEnum)
        ...
        {
			string[] names = m_lister.GetNames( context );
			DefineEnum( names, null );
            ...
            Sce.Atf.Applications.SkinService.ApplyActiveSkin(ctrl);
            return ctrl;
        }
		...
		private DynamicLongEnumEditorLister m_lister;
