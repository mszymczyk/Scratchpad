#include "ZMQHubCommunication.h"
#include <Core\Util\ZMQHubUtil.h>
	...
	namespace SettingsEditorZMQ
	{
		void handleMessage( const IncommingMessage& msg )
		{
			...
			if (cmd == "setInt")
			{
				...
				SettingsEditor::_internal::updateParam( settingsFile.c_str(), groupame.c_str(), presetName.c_str(), paramName.c_str(), &iVal, nValues );
			}
			else if (cmd == "setFloat")
			{
				...
				SettingsEditor::_internal::updateParam( settingsFile.c_str(), groupame.c_str(), presetName.c_str(), paramName.c_str(), f, nValues );
			}
			else if (cmd == "setString")
			{
				std::string sVal = msg.readString( ok );
				SettingsEditor::_internal::updateParam( settingsFile.c_str(), groupame.c_str(), presetName.c_str(), paramName.c_str(), sVal.c_str() );
			}
			else
			{
